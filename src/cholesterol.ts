...
export function cholesterolChart() {
  ...
  d3.csv("/src/cholestBinned.csv").then((d) => {
    ...
    d.forEach((x) => {
      ...
      almost.push(
        parseInt(x.numAlmostHighTrig === undefined ? "" : x.numAlmostHighTrig)
      );
      ldlamts.push(parseInt(x.numHighLDL === undefined ? "" : x.numHighLDL));
    });
    xScale.domain(bins);
    yScale.domain([0, d3.max(amts)!]);
    const I = d3.range(bins.length);
  ...
  svg
    ...
    .attr(
      "transform",
      `translate(${-width / 2 - margin.left - 35},${height / 2}) rotate(-90)`
    )
    .text("Count");
